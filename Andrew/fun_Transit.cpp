...
v[0] = V("FlippingCoin");
if(v[0]==0)
 V("InvestmentDecision"); //this line ensures that InvestmentDecision is completed before the code for this variable is completed

v[1]=V("GreenInvestment");
...
WRITELS(cur2,"DebtF",0, t);
WRITELS(cur2,"NetWorth",10, t);
WRITELS(cur2,"sBalanceF",10, t);
...
WRITELS(cur1,"Ms",0, t);
WRITELS(cur1,"CapitalCapacity",10, t);
WRITELS(cur1,"MovAvProfit",10, t);
WRITELS(cur1,"Stocks",10, t);
WRITELS(cur1,"price",0.5*V("AvPrice"), t);
WRITELS(cur1,"backlogSales",0, t);
WRITELS(cur1,"Profit",10, t);
WRITELS(cur1,"MsSector",0, t);
WRITELS(cur1,"NumK",0, t);
WRITELS(cur1,"MovAvExpSales",1, t);
WRITELS(cur1,"LaborForce",10, t);
...
WRITELS(cur4,"K",10,t);
WRITELS(cur4,"KExpenditures",10,t);
...
INCRS(p->hook->up,"BLrecouped",v[55]); //REMOVED RECOUP
...
v[4]=V("coefMarkupVar");
v[5]=V("backlog");
v[10]=V("DesiredStocks");
if(v[1]==0 || v[2]==0 || v[0]>0 )
 v[3]=1+V("minMarkup");//normal level of markup
else
	{
 	//v[3]=1+V("minMarkup")+v[4]*log(1+(v[1]+v[5])/v[2]);
 	//v[3]=1+V("minMarkup")+v[4]*log(1+(v[1]+v[5]-v[0])/v[2]);
 	//v[3]=1+V("minMarkup")+v[4]*log(1+(v[1])/v[2]); // without backlogs
 	v[3]=1+V("minMarkup")+v[4]*log(1+(v[1]+v[5]-v[10]*v[1]+v[0])/v[2]);
	}
...
Hyperbolic equation: U = Constant + beta/V (B̦rsch-Supan: c<1, beta=6)
...
//v[4]=(double)t;
  ...
  v[1]=V("GdpCGrowth");
  //INTERACT("MoAvGdpCGrowth",v[1]);
  v[0]=VL("MoAvGdpCGrowth",1);
  //INTERACT("GdpCGrowth1",v[0]);
  v[2]=0.9*v[0]+0.1*v[1];
  //INTERACT("MoAv",v[2]);
